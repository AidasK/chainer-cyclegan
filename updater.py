...
class Updater_SimGAN_gt_l1(chainer.training.StandardUpdater):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._buffer_x = HistoricalBuffer(self._max_buffer_size, self._image_size)
        self._buffer_y = HistoricalBuffer(self._max_buffer_size, self._image_size)
        super(Updater_SimGAN_gt_l1, self).__init__(*args, **kwargs)
...
class Updater_gt_l1_fs(chainer.training.StandardUpdater):
    ...
    def update_core(self):
        ...
        x_gt_maps =xp.stack(x_gt_maps)
        y = xp.stack(y_images)
        gt_mb_locs = xp.stack([xp.asarray(gt_coded[i][0]) for i in range(len(gt_coded))])
        gt_mb_labels = xp.stack([xp.asarray(gt_coded[i][1]) for i in range(len(gt_coded))])
        ...
        loss_fs = (loc_loss * self.alpha + conf_loss)*self._lambda_fs
        ...
        chainer.report({'loss_gen_gt_l1': loss_gen_g_gt_l1}, self.gen_g)
        chainer.report({'loss_gen_gt_l1': loss_gen_f_gt_l1}, self.gen_f)
        chainer.report({'loss_fs': loss_fs}, self.gen_g)
        # if self._cfmap_loss != None:
        #     chainer.report({'loss_cfmap_X': loss_cfmap_X})
        #     chainer.report({'loss_cfmap_Y': loss_cfmap_Y})

        loss_gen = loss_gen_g_adv + loss_gen_f_adv + loss_cycle_x + loss_cycle_y + loss_idt + loss_gen_gt_l1 + loss_fs#+ loss_cfmap_gen
        loss_gen.backward()
